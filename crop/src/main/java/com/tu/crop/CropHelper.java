...
public class CropHelper {
  ...
  public static void handleResult(CropHandler handler, int requestCode, int resultCode,
      Intent data) {
    ...
    if (resultCode == Activity.RESULT_CANCELED) {
        ...
        case REQUEST_GALLERY:
          ...
          if ("false".equals(handler.getCropParams().crop)) {
            ...
            if (!new File(uri.getPath()).exists()) {
              ...
              try {
                String fileName = BitmapUtil.saveFile(context,
                    BitmapUtil.compressImage(uri.getPath(), 800f, 480f, 100));
      ...
      handler.startActivityForResult(intent, REQUEST_CROP);
    }
  }
  ...
  public static Intent buildCropIntent(String action, CropHandler handler) {
    ...
    Intent intent = new Intent(action, null).setDataAndType(params.uri, params.type)
    ...
    handler.getCropParams().cropResult =
        Uri.fromFile(new File(getCropFilePath(handler.getContext())));
  ...
  private static String getCropCacheDir(Context context) {
    ...
    if (file == null) file = context.getCacheDir();
  ...
  @SuppressLint("NewApi") public static Uri uriFormat(final Context context, final Uri uri) {
